...
async def test_state_registry_backend_fails_closed_then_delegates_every_operation() -> None:
    ...
    backend.resolve_release_workspace.return_value = "workspace"
    backend.resolve_operation_workspace.return_value = "workspace"
    backend.resolve_installable_release_id.return_value = "release"
    ...
    assert await adapter.resolve_release_workspace("id") == "workspace"
    assert await adapter.resolve_operation_workspace("id") == "workspace"
    assert await adapter.resolve_installable_release_id(artifact_id="id") == "release"
